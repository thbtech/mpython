...
def main():
    ...
    tests_natmod = [
        file
        for file in glob.iglob("extmod/*.py", root_dir="tests")
        if file.split("/")[1].startswith(NATMOD_LIBS)
    ]
    tests_multi_net = [file for file in glob.iglob("multi_net/*.py", root_dir="tests")]
    tests_multi_bluetooth = [
        file for file in glob.iglob("multi_bluetooth/*.py", root_dir="tests") if "/ble_" in file
    ]
